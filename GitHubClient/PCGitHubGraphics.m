...
static UIImage* _github_icon = nil;
...
static UIImage* _imageOfGitHubLogin = nil;
...
+ (UIImage*)github_icon { return _github_icon ?: (_github_icon = [UIImage imageNamed: @"github_icon"]); }
...
+ (void)drawProgressBarWithRotation: (CGFloat)rotation;
{
    ...
    UIBezierPath* ovalPath = [UIBezierPath bezierPathWithOvalInRect: CGRectMake(-12.5, -12.5, 25, 25)];
    [UIColor.whiteColor setFill];
    [ovalPath fill];
    [UIColor.blueColor setStroke];
    ovalPath.lineWidth = 2.5;
    CGFloat ovalPattern[] = {3.5, 5};
...
+ (void)drawGitHubLogin;
{
    ...
    //// Color Declarations
    UIColor* gitHubGreen = [UIColor colorWithRed: 0.167 green: 0.915 blue: 0.904 alpha: 1];
    CGFloat gitHubGreenRGBA[4];
    [gitHubGreen getRed: &gitHubGreenRGBA[0] green: &gitHubGreenRGBA[1] blue: &gitHubGreenRGBA[2] alpha: &gitHubGreenRGBA[3]];

    UIColor* gitHubShadow = [UIColor colorWithRed: (gitHubGreenRGBA[0] * 0.5) green: (gitHubGreenRGBA[1] * 0.5) blue: (gitHubGreenRGBA[2] * 0.5) alpha: (gitHubGreenRGBA[3] * 0.5 + 0.5)];

    //// Gradient Declarations
    CGFloat gitHubLocations[] = {0, 1};
    PCGradient* gitHub = [PCGradient gradientWithColors: @[gitHubGreen, gitHubShadow] locations: gitHubLocations];

    //// Rectangle Drawing
    CGRect rectangleRect = CGRectMake(0, 0, 240, 40);
    UIBezierPath* rectanglePath = [UIBezierPath bezierPathWithRoundedRect: rectangleRect cornerRadius: 5];
    CGContextSaveGState(context);
    [rectanglePath addClip];
    CGContextDrawLinearGradient(context, gitHub.CGGradient, CGPointMake(120, -0), CGPointMake(120, 40), 0);
    CGContextRestoreGState(context);
    {
        NSString* textContent = @"Login with GitHub";
        UIFont* rectangleFont = [UIFont fontWithName: @"Arial-BoldMT" size: UIFont.smallSystemFontSize];
        [UIColor.whiteColor setFill];
        [textContent drawInRect: CGRectOffset(rectangleRect, 0, (CGRectGetHeight(rectangleRect) - [textContent sizeWithFont: rectangleFont constrainedToSize: rectangleRect.size lineBreakMode: UILineBreakModeWordWrap].height) / 2) withFont: rectangleFont lineBreakMode: UILineBreakModeWordWrap alignment: UITextAlignmentCenter];
    }


    //// Oval Drawing
    UIBezierPath* ovalPath = [UIBezierPath bezierPathWithOvalInRect: CGRectMake(21, 6, 30, 30)];
    CGContextSaveGState(context);
    [ovalPath addClip];
    CGContextScaleCTM(context, 1.0, -1.0);
    CGContextDrawTiledImage(context, CGRectMake(21, -6, PCGitHubGraphics.github_icon.size.width, PCGitHubGraphics.github_icon.size.height), PCGitHubGraphics.github_icon.CGImage);
    CGContextRestoreGState(context);
}
...
{
    if (_imageOfGitHubLogin)
        return _imageOfGitHubLogin;

    UIGraphicsBeginImageContextWithOptions(CGSizeMake(240, 40), NO, 0.0f);
    [PCGitHubGraphics drawGitHubLogin];
    _imageOfGitHubLogin = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
...
- (void)setGitHubLoginTargets: (NSArray*)gitHubLoginTargets
{
    _gitHubLoginTargets = gitHubLoginTargets;

    for (id target in self.gitHubLoginTargets)
        [target setImage: PCGitHubGraphics.imageOfGitHubLogin];
}
